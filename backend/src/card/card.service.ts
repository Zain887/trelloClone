...
export class CardService {
  ...
  async findAllCardsByListId(listId: string): Promise<CardEntity[]> {
    const list = await this.listService.findOrThrowError(listId);
    // console.log(list.id);
    
    return await this.cardRepository.find({ where: { list: { id: list.id } } });
    // return await this.listRepository.find({ where: { board: { id: board.id } } })
  }
