...
export class MemberService {
  ...
  async removeMemberFromBoard(boardId: string, email: string) {
    try {
      console.log(boardId, email);

      const boardMember = await this.boardMemberRepository.findOne({ where: { board: { id: boardId }, email: email } });
      console.log(boardMember);

      if (!boardMember) {
        throw new NotFoundException(`Member with email ${email} is not associated with board ${boardId}`);
      }
      await this.boardMemberRepository.remove(boardMember);

      // Ab member table se member ko bhi delete karein
